...
==================
  ...
  1. Why is it necessary to implement a change directory 'cd' command in
	...
	'cd' is something that is executed internally withing its own shell previously 
	it was called a external program called 'chdir' commad.

  2. Explain how our sample shell implements the change directory command.

	it does it by comapring two strings usually the program name/ command line 
	argument and 'cd'. if they are not the same thing the we are going to use
	static int is grater than 1
  ...
  3. What would happen if this program did not use the fork function, but
	...
	it replacces the running program while it also loads a new program into the process.

  4. Explain what the return value of fork() means and how this program
     uses it.(used web page geekforgeeks.com)

	fork() - it transfer the pid of the child process witch will passes 
	it to it's parent process.
	1. porsitve # - returned to the parent.
	2. negitive # - no child process created
	3. value # of 0 means - it returns created child process
  ...
  5. What would happen if fork() were called prior to chdir(), and chdir()
	...
	if the child process is successfully created you will need to change 
	directory to it's child. It will faild to change directory because we
	are in the latest process 

  6. Can you run multiple versions of ./b.sh in the background?
     What happens to their output?

	output overlaps so answer is yes

  7. Can you execute a second instance of our shell from within our shell
     program (use './shell')?  Which shell receives your input?

	yes you can execute a second instance. The shell receives a input but you can not 
	be in 3 instances of the shell inthis currecnt program 

  8. What happens if you type CTRL-C while the countdown script ./b.sh is
     running?  What if ./b.sh is running in the background?
	
	so first it will kills the running process then create a new one even if
	./b.sh while it's running in the background. 
  ...
  9. Can a shell kill itself?  Can a shell within a shell kill the parent
	...
	yes a shell withing a shell can kill the parent shell withought killing itself.
  ...
  10. What happens to background processes when you exit from the shell?
	...
	it could still run but the background processes might die if they try accessing
	a non-existent terminal.and you can see them using P1 and or ps -o pid.
