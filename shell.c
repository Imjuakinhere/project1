...
void imtheparent(pid_t child_pid, int run_in_background)
{
        ...
        fprintf(stderr,
                        "  Parent says 'wait() returned so the child with pid=%dis finished.'\n",
                        child_pid);
...
int main(int argc, char **argv)
{
        ...
        while (1)
        {
                ...
                // TO-DO P5.2
                fprintf(stdout, "Shell(pid=%d)> ", shell_pid, counter);
                fflush(stdout);
                ...
                if (*buffer == '!' && counter <= 9)
                {
                        ...
                        {
                                strcpy(buffer, lastcmd[i - 1]);
                                lastcmd[counter - 1] = lastcmd[i - 1];
                        }
                }
                else if (strlen(buffer) > 2)
                {
                        lastcmd[counter - 1] = malloc(strlen(buffer) + 1);
                ...
                buffer[i] = '\0'; /* Terminate input, overwriting the '&' if itexists */
